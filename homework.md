...
### 破損データの draft 復元（S1-3）
- 起動時にパースできなかった保存データは `:broken-<timestamp>` キーへ退避する所までは実装済み（破壊は止めた）。
- より親切な形: サンプルを出しつつ、退避した生テキストを CodePane の draft に流し込みエラー表示し、ユーザーが自分で直せるようにする。既存の draft 機構（`{text, base}`）とエラー表示がそのまま使える。

### ストレージキーの粒度
- `listStoredEntries` は `gmermaid:` 全部を図として扱う。設定等を置く前に図データを `gmermaid:doc:` 配下へ分離する（キー移行を伴うので早いうちに）。

### タブ間同期
- 2タブで同じ図種を開くと同じ autosave キーを後勝ちで潰し合う。`storage` イベントの購読か、少なくとも既知の制約として明記する。Files パネルも開いている間は他タブの保存を反映しない（削除時のみ再取得）。
